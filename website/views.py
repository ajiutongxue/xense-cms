...
def index(request):
    focus_list = Focus.objects.filter(is_published=True).order_by('-rank_num')
    activities = Post.objects.filter(
        category__full_name='activities-default',
        is_published=True
    ).order_by('-rank_num')[:3]
    calendars = Calendar.objects.filter(is_published=True).order_by('-rank_num', '-active_time')
    research_list = Post.objects.filter(category__full_name='research-platform', is_published=True).order_by('-rank_num', '-pub_date')[:4]
    honor_list = Gallery.objects.filter(category__full_name='achievements-honors', is_published=True).order_by('-rank_num')[:4]
    friend_links = SimplePost.objects.filter(category__full_name='friend-links', is_published=True).order_by('-rank_num')
...
def about(request, slug='introduction'):
    ...
    cate = Category.objects.get(full_name='single-about')
    tmpl = _get_page_tmpl('default-article', cate, page)
    brother_cate_list = SinglePage.objects.filter(category__full_name='single-about').order_by('-rank_num')
    context = {
        'page': page,
...
def contact(request):
    ...
    cate = Category.objects.get(full_name='single-contact')
    tmpl = _get_page_tmpl('tmpl-contact', cate, page)
    brother_cate_list = SinglePage.objects.filter(category__full_name='single-contact').order_by('-rank_num')
...
def list_page(request, parent_cate, cate_slug, page_num=1):
    cate = Category.objects.get(slug=cate_slug)
    tmpl = 'tmpl-list'
    ...
    brother_cate_list = Category.objects.filter(full_name__startswith=parent_cate).order_by('-rank_num')
...
def article(request, parent_cate, cate_slug, pid, slug):
    ...
    relative_list = Post.objects.filter(category__full_name=page.category.full_name, is_published=True).exclude(id=pid)[
                    :6]
    context = {
        'article': page,
        # 'brother_cate_list': brother_cate_list,
